...
model: sonnet
allowed-tools: Bash(python3:*), Bash(pnpm:*), Bash(git status:*), Bash(git diff:*), Bash(git log:*), Read, Write, Edit, Grep, Glob
argument-hint: [what you are designing or reviewing]
---
...
## In this repository, read this before running anything

**This frontend already has a design system, and it wins.** The visual language lives in
`apps/frontend/src/app/globals.css` — the shadcn/ui token names repointed at a "soft ledger" palette
(warm paper canvas, white `.panel` surfaces, three semantic money tints, pill-shaped controls). See
`apps/frontend/CLAUDE.md` § "Design system" for the full rules and the reasoning behind each value.

That changes how this skill is used here. It is a source of **UX rules and review criteria**, not a
source of new palettes:

- **Use it for**: the priority-1→10 checks below, `--domain ux` (98 guidelines), `--domain icons`,
  `--domain chart`, `--domain gsap`, `--stack nextjs|shadcn|react`, and the
  `references/quick-reference.md` / `references/pro-rules.md` checklists.
- **Do not adopt its colour, typography, or spacing output.** `--design-system` emits raw hex
  palettes and its own `--color-*` / `--space-*` variable tables. Pasting those into a component
  violates this repo's rule that **a hex in a component is a bug** — reach for `bg-card`,
  `text-muted-foreground`, `bg-secondary`, and the `income`/`balance`/`expense` tints instead. If a
  token genuinely does not exist yet, add it to `globals.css` and derive it against the **canvas**,
  not against a white panel.
- **Do not run `--persist`.** It writes `design-system/<slug>/MASTER.md` at the output root, which
  would stand up a second, competing source of truth next to `globals.css` and a new top-level
  directory in the Turborepo. If you want a written rationale, put it in `globals.css` comments or
  `apps/frontend/CLAUDE.md`, where the existing decisions already live.
- **Contrast ratios are measured against the canvas** (`#efeee9`), which is darker than the white
  panels. A value that passes on `bg-card` can fail on the page — this is a documented past bug, not
  a hypothetical.

Where this skill's generic advice and `apps/frontend/CLAUDE.md` disagree, `CLAUDE.md` is correct;
several of its rules exist because the generic advice was tried and failed an audit here.
...
This skill is installed in the repository at `.claude/skills/ui-ux-pro-max/`, so the paths below are
repo-relative — run them from the repository root:

```bash
python3 ".claude/skills/ui-ux-pro-max/scripts/search.py" "<query>" --domain <domain>
```

`scripts/` and `data/` must stay siblings: `core.py` resolves the CSVs as
`Path(__file__).parent.parent / "data"`, so the script works from any working directory when invoked
by path, but moving either folder alone breaks every query. From a subdirectory, use an absolute
path rather than a relative one.

`python3`, not `python` — this repo's toolchain does not put a `python` shim on `PATH`. No external
dependencies; verified against the Python 3.9.6 that ships with macOS.
...
- **Stack**: already known for this repository — `nextjs` for app/routing/rendering questions, `shadcn`
  for component-primitive questions, `react` for re-render and list-performance questions. Pass one of
  those to `--stack` explicitly. Do not re-derive it from `package.json`, and never pass
  `html-tailwind`: this frontend is Next.js with shadcn/ui primitives, and that default silently
  misroutes every recommendation.

### Step 2: Generate Design System (REQUIRED for new pages/projects)

> **Not required in this repository** — the design system already exists in
> `apps/frontend/src/app/globals.css` (see "In this repository" above). Read that file and
> `apps/frontend/CLAUDE.md` § "Design system" instead. Run `--design-system` here only to sanity-check
> a *pattern* or section ordering, and ignore its colour/typography/spacing output.
...
> **Do not use `--persist` in this repository.** It would create a second source of truth competing
> with `globals.css`. This section is kept only so the flag's behaviour is documented if you meet it
> in another project.
