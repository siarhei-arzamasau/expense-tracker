...
This file provides guidance to coding agents when working in `apps/backend`. The
repository-wide rules — output language, git branches, commit messages, workspace commands, and the
cross-cutting architecture decisions — live in the root `AGENTS.md` and still apply here.
...
- The service layer converts money input with `.toFixed(2)` rather than passing raw JS numbers to Prisma. See the money contract in the root `AGENTS.md`.
