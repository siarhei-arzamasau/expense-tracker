...
/**
 * A panel floating on the canvas: no border, a large radius, and a shadow that
 * is mostly a soft ambient pool rather than a drop. Borders return only where a
 * card has to signal something (the destructive card on /profile).
 */
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground rounded-panel shadow-panel flex flex-col gap-6 py-7",
        className,
      )}
      {...props}
    />
